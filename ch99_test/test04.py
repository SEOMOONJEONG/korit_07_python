...
김삼: 1표
'''
can_num = int(input(f'후보자 수를 입력하시오 >>> '))
...
vote_num = int(input(f'전체 투표 횟수를 입력하시오 >>> '))

for i in range(vote_num):
    vote = input(f"{i + 1}번째 투표 (1: {candidates[0]}, 2: {candidates[1]}, 3: {candidates[2]}) >>> ")
    if vote == '1':
...
for i in range(can_num):
    print(f'{candidates[i]}: {votes[candidates[i]]}표')
